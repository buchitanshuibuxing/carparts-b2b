...
export class InventoryService {
  ...
  async findAll(page = 1, pageSize = 20, filters?: { warehouse_zone?: string; category?: string; is_low_stock?: boolean; keyword?: string }) {
    ...
    qb.orderBy("inv.updated_at", "DESC").skip((page - 1) * pageSize).take(pageSize);
    const [items, total] = await qb.getManyAndCount();

    // Batch fetch parts for enrichment
  ...
  async getLowStock(limit = 50) {
    const items = await this.inventoryRepo.createQueryBuilder("inv")
    ...
    return items.map(inv => {
      const part = partMap.get(inv.partId);
      return { ...inv, oe_number: part?.oeNumber || "", part_name_cn: part?.partNameCn || "" };
    }).filter(inv => {
      const part = partMap.get(inv.partId);
